...
constraint fk_from_address_id foreign key (from_address_id) 
...
constraint fk_car_id1 foreign key (car_id)
	references cars(id),
...
You will have to insert records of data into the clients table, 
...
select 
    ...
    concat('(088) 9999', cast((id * 2) as char)) as `phone_number`
from drivers
where id >= 10
	and id <= 20;
...
The cars  must have a mileage greater than 800000 (inclusive) 
	or NULL 
    and must be older than 2010(inclusive).
...
# 04
/*Delete all clients from clients table
	that do not have any courses 
...
Order the results descending by the count of courses, 
	then by the car’s id. 
...
from cars cr
	left join courses cs on cr.id = cs.car_id
group by cr.id
having `count_of_courses` <> 2
order by `count_of_courses` desc, cr.id;
...
Select ... and total sum of all courses.
...
*/
select cl.full_name, count(cr.id) as `count_of_cars`, sum(cr.bill) as `total_sum`
from clients cl
	join courses cr on cl.id = cr.client_id
where left(cl.full_name, 2) like '%a'
group by cl.id 
...
# 09
/*courses information that they need is the address
...
select ad.`name` as `name`,
	(case 
    when hour(cr.`start`) >= 6 and hour(cr.`start`) <= 20 then 'Day'
	when hour(cr.`start`) > 20 or hour(cr.`start`) < 6 then 'Night'
	end) as `day_time`,
    cr.bill,
    cl.full_name,
    cars.make,
...
from courses cr
	join addresses ad on cr.from_address_id = ad.id
...
# 11
/*Extract data about the addresses with the given address_name. The needed data is the 
	name of the address, 
    full name of the client, 
	level of bill (depends of course bill – 
		Low – lower than 20(inclusive)
		Medium – lower than 30(inclusive), and 
        High)
	make and 
    condition of the car and the 
...
begin
    select addresses.`name`,
        ...
        when courses.bill > 20 and courses.bill <= 30 then 'Medium'
