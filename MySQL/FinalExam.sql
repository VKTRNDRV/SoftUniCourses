...
create table students(
    ...
    city_id int,
constraint fk_city_id1 foreign key (city_id)
	references cities(id)
);
...
# 02
/*insert records of data into the courses table, based on the courses table.
For courses with id equal or lesser than 5
insert data in the course table with the following values:
...
# 03
/*Raise the tuition fee by 300 for universities 
	with id equal or greater than 5 
    and less than 12 (inclusive).*/
update universities
...
# 04
/*Delete all universities for which we
	don’t have information about the number of staff.*/
...
Order the results descending by first_name, 
	then by email ascending, 
    then by id in ascending order and 
...
# 07
/*write a query that returns: full_name, username and password 
for students who do not have any assigned course.
...
# 08
/*Extract the students_count (total number of assigned students)
	in each university and the corresponding university_name.
Get only those universities with students_count equal or greater than 8. 
Order the results descending by students_count and 
	then by university_name in descending order. 
students_count	university_name
15				Haus und Landwirtschaftliche Schule
12				Penn
12				Nikolaus-von-Kues-Gymnasium
11				Lycée Thiers
. . . 				. . . 
8				McGill University
*/
...
# 09
/*extract the university_name, city_name, address, price_rank and tuition_fee.
...
Required Columns
•	price_rank (less than 800 – "cheap", 
equal or above 800 and less than 1200 – "normal", 
equal or above 1200 and less than 2500 – "high", 
...
that receives a course name 
and returns the average grades of the grades 
	from those students that are graduated.
Required Columns
•	course_name
...
WHERE c.name = 'Quantum Physics'; # 5.80 #
...
# 11
/*Create a stored procedure udp_graduate_all_students_by_year
which accepts the following parameters:
•	year_started INT
Extracts data about all courses that started on the given year,
find the assigned students
and change their graduated status to true
*/
delimiter $$
