...
create table users_photos(
	user_id int not null,
    photo_id int not null,
constraint fk_us_id foreign key (user_id)
	references users(id),
constraint fk_ph_id foreign key (photo_id)
	references photos(id)
);
...
create table likes(
    ...
    photo_id int,
    user_id int,
constraint fk_pht_id foreign key (photo_id)
	references photos(id),
...
insert into addresses (id, address, town, country, user_id) values (1, '97 Valley Edge Parkway', 'DivinÃ³polis', 'Brazil', 74);
...
insert into addresses (id, address, town, country, user_id) values (21, '028 Browning Junction', 'San Pedro SacatepÃ©quez', 'Guatemala', 84);
...
insert into addresses (id, address, town, country, user_id) values (34, '0 Dawn Trail', 'ItajubÃ¡', 'Brazil', 40);
...
insert into addresses (id, address, town, country, user_id) values (48, '77615 Mosinee Plaza', 'Pâ€™rimorskâ€™oe', 'Georgia', 85);
insert into addresses (id, address, town, country, user_id) values (49, '71 Emmet Circle', 'Al Wudayâ€˜', 'Yemen', 76);
...
insert into addresses (id, address, town, country, user_id) values (58, '015 Goodland Circle', 'LwÃ³wek ÅšlÄ…ski', 'Poland', 76);
insert into addresses (id, address, town, country, user_id) values (59, '64236 Maple Wood Junction', 'Longtouâ€™an', 'China', 100);
...
insert into addresses (id, address, town, country, user_id) values (63, '8 Blackbird Trail', 'NÃ©a RÃ³da', 'Greece', 94);
...
insert into addresses (id, address, town, country, user_id) values (66, '2 Ridge Oak Terrace', 'Thá»‹ Tráº¥n YÃªn CÃ¡t', 'Vietnam', 6);
insert into addresses (id, address, town, country, user_id) values (67, '9 Village Avenue', 'CriaÃ§Ã£o Velha', 'Portugal', 1);
insert into addresses (id, address, town, country, user_id) values (68, '90972 Burning Wood Junction', 'Haumeni', 'Indonesia', 78);
insert into addresses (id, address, town, country, user_id) values (69, '73 Northland Alley', 'DomaÅ¡inec', 'Croatia', 26);
...
insert into addresses (id, address, town, country, user_id) values (89, '8184 Canary Center', 'PaulÃ­nia', 'Brazil', 65);
...
insert into addresses (id, address, town, country, user_id) values (92, '6 Warrior Road', 'Coxâ€™s BÄzÄr', 'Bangladesh', 85);
...
insert into addresses (id, address, town, country, user_id) values (99, '43 Sommers Street', 'Komsomolâ€™skiy', 'Russia', 96);
...
update addresses
set country = (case 
	when left(country, 1) = 'B'
		then 'Blocked'
	when left(country, 1) = 'T'
		then 'Test'
	when left(country, 1) = 'P'
		then 'In Progress'
	end
	)
...
# 04
/*As you remember at the beginning of our work, 
we inserted and updated some data. Now you need to remove some addresses.	
...
# 06
/*Extract the 5 most commented photos with their count of comments. 
...
…		…						…																	…
*/
select ph.id, ph.`date` as `date_and_time`, ph.`description`, count(cm.id) as `commentsCount`
from photos ph
	join comments cm on ph.id = cm.photo_id
group by ph.id
...
…				…
*/
select concat_ws(' ', cast(u.id as char), u.username) as `id_username`, u.email
from users u
...
# 08
/*Extract photos id with their likes and comments. 
...
from photos ph
	left join likes lk on ph.id = lk.photo_id
    left join comments cm on ph.id = cm.photo_id
group by ph.id
...
# 09
/*Extract those photos that their upload day is 10 and summarize their description. 
...
begin
    ...
    select count(distinct ph.id) into photos_count
    from users us
		join users_photos up on us.id = up.user_id
        join photos ph on up.photo_id = ph.id
	where us.username = username_param;
...
that increase the age of the user by 10 years only if the given user exists. 
