...
constraint fk_waiter_id foreign key (waiter_id)
...
/*insert into the products table
	based on the waiters table.
For waiters with id greater than 6
insert data in the products table with the following values:
...
# 05
/*Extract info about the clients.
Order the results by birthdate in descending order
	and id in descending;
Required Columns
Example
...
# 07
/*Write a query that returns: username and password for all waiters 
	which are not fired (fired waiter is a waiter without salary)
the username is generated by their last name
	immediately followed by their first name
	followed by the number of characters from first name 
	and at the end "Restaurant".
...
*/
select concat(last_name, first_name, cast(char_length(first_name) as char), 'Restaurant') as `username`,
...
# 08
/*Extract from the database the id(product), the name and the count of products from all orders with this name
	where the count is greater or equal to 5.
...
/*Write a query that returns the table_id, capacity, count_clients and availability of all tables from the 1st floor.
	...
	Availability is based on how many people are sitting and the capacity of the table.
		If the capacity is greater than count_clients than it should be "Free seats", if the capacity is equal to the count_clients it should be "Full"
...
WHERE c.first_name = 'Silvio' AND c.last_name= 'Blyth'; # 96.90 #
...
Extracts data about the products from the given type and reduces the prices by 20% of all products 
	which have price higher or equal to 10.00
    and are from the given type.
*/
delimiter $$
create procedure udp_happy_hour(type_param varchar(50))
begin
	update products
    set price = price * 0.8
    where price >= 10
